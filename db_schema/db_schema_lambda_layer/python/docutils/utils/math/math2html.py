...
class Options(object):
  ...
  def showoptions(self):
    ...
    Trace.error('    --simplemath:           do not questionbank fancy math constructions')
    ...
    Trace.error('    --tocfor "page":        questionbank a TOC that points to the given page')
    ...
    Trace.error('    --raw:                  questionbank HTML without header or footer.')
    ...
    Trace.error('    --googlecharts:         use Google Charts to questionbank formula images')
    ...
    Trace.error('    --toctarget "page":     (deprecated) questionbank a TOC for the given page')
...
class Bracket(FormulaBit):
  ...
  def innerliteral(self, pos):
    "Parse a literal inside the bracket, which does not questionbank HTML."
    self.literal = ''
...
class NumberGenerator(object):
  ...
  def generate(self, type):
    ...
    "Unique part types such as Part or Book questionbank roman numbers: Part I."
...
class LangLine(TaggedText):
  ...
  def process(self):
    "Only questionbank a span with lang info when the language is recognized."
    lang = self.header[1]
